#!/bin/bash
# Top-level launcher for the KuaRec Wukong PACEvolve task.
...
WUKONG_EVAL_TIMEOUT="${WUKONG_EVAL_TIMEOUT:-420}"
WUKONG_RECOMPILE_TIMEOUT="${WUKONG_RECOMPILE_TIMEOUT:-60}"
...
export PACEVOLVE_ROLLOUT_MAX_RESPONSE_LEN="${PACEVOLVE_ROLLOUT_MAX_RESPONSE_LEN:-8192}"

# Default 16-GPU split: 4 train + 8 rollout + 4 eval.
...
export PACEVOLVE_ROLLOUT_NUM_GPUS="${PACEVOLVE_ROLLOUT_NUM_GPUS:-8}"
export PACEVOLVE_ROLLOUT_NUM_GPUS_PER_ENGINE="${PACEVOLVE_ROLLOUT_NUM_GPUS_PER_ENGINE:-8}"
export PACEVOLVE_EVAL_GPU_IDS="${PACEVOLVE_EVAL_GPU_IDS:-12,13,14,15}"
export PACEVOLVE_MAX_CONCURRENT_EVALS="${PACEVOLVE_MAX_CONCURRENT_EVALS:-1}"
...
TASK_ROOT="${ROOT_DIR}/pacevolve/tasks/${WUKONG_TASK_ID}"
BASE_CONFIG_YAML="${TASK_ROOT}/config/config_${WUKONG_CONFIG_ID}.yaml"
...
echo "=== Wukong + PACEvolve Configuration ==="
echo "RUN_NAME: ${RUN_NAME}"
echo "TASK_ID: ${WUKONG_TASK_ID}"
echo "TASK_NAME: ${TASK_NAME}"
echo "CONFIG_YAML: ${RUNTIME_CONFIG_YAML}"
echo "DATA_PATH: ${WUKONG_DATA_PATH}"
