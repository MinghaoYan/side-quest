"""Prompts for the KuaRec Wukong task."""
...
def _load_editable_block() -> str:
    src_path = Path(__file__).resolve().parents[1] / "src" / "train_wukong.py"
    ...
    if not match:
        raise ValueError("Could not locate editable block in train_wukong.py")
    return match.group(1).strip()


KUAIREC_WUKONG = _load_editable_block()

CODING_REQ = """
While completing your task, you MUST:
- Enclose your code in triple backticks to properly format the code in Markdown.
- Your code will replace the content between RegexTagCustomPruningAlgorithmStart and RegexTagCustomPruningAlgorithmEnd in train_wukong.py.
...
BACKGROUND = """
### Background on the KuaRec Wukong benchmark

This task adapts the Wukong recommendation backbone to sequential recommendation on KuaRec, a Kuaishou dataset.
...
1. **Feature design**
   Turn a user history into sparse fields and dense statistics.

2. **Model architecture**
   Use Wukong-style field interaction layers, pooling, residual structure, and item-scoring logic to produce better user representations.

Because the trainer is fixed, this is not a hyperparameter game. Good candidates usually come from better sequence summarization, better feature namespaces, better pooling, or better interaction design.
"""

TASK_INTRO = """
You are an expert in recommender systems, especially feature crosses, sequence modeling, and efficient large-scale ranking models. Your goal is to improve a Wukong-style sequential recommender under a fixed KuaRec training budget.
"""
...
def construct_mutation_prompt(sota_algorithm, ablation_list):
    ...
    return f"""
We are conducting an evolutionary optimization process for the KuaRec Wukong recommendation benchmark.
...
- Better sparse feature namespaces and bucketization
- Better Wukong interaction blocks or residual mixing
- Better pooling from field tensors into a user embedding
