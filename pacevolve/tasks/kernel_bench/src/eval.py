...
def eval_kernel_against_ref(
    ...
    print(f"Correctness: {kernel_exec_result.correctness}, Runtime: {kernel_exec_result.runtime}, Baseline Runtime: {baseline_time}, Kernel speedup: {baseline_time/kernel_exec_result.runtime}")
...
def time_execution_with_cuda_event(
    ...
    *args,
    ...
    elapsed_times = []
    num_flush_elements = 256*1024*1024
    
    # Create the dummy tensor for flushing
    try:
        flush_tensor = torch.empty(num_flush_elements, dtype=torch.float32, device=device)
    except RuntimeError as e:
        # This can happen if L2 cache is huge and we go OOM.
        # Handle this gracefully, e.g., by using a smaller tensor.
        print(f"Warning: Failed to allocate L2 flush tensor ({e}). Proceeding without flush.")
        flush_tensor = None

    # Actual trials
    for trial in range(num_trials):
        if flush_tensor is not None:
            flush_tensor.fill_(0)
