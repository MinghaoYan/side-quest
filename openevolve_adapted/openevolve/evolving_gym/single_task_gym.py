...
from openevolve.puct_archive import PUCTArchive
...
class SingleTaskEvolvingGym:
    ...
    def __init__(self,
                 ...
                 use_puct_reuse: bool = False,
                 puct_exploration_coef: float = 1.0,
                 puct_archive_size: int = 1000,
                 puct_top_children_per_parent: int = 2):
        """
        ...
        # PUCT archive for TTT-Discover reuse
        self.use_puct_reuse = use_puct_reuse
        self.puct_archive: Optional[PUCTArchive] = None
        if use_puct_reuse:
            self.puct_archive = PUCTArchive(
                exploration_coef=puct_exploration_coef,
                archive_max_size=puct_archive_size,
                top_children_per_parent=puct_top_children_per_parent,
            )
            print(f"[SingleTaskEvolvingGym] PUCT reuse enabled: "
                  f"c={puct_exploration_coef}, archive_size={puct_archive_size}, "
                  f"top_children={puct_top_children_per_parent}")
    ...
    def problem_generator_puct(self) -> Tuple[Dict[str, str], Program]:
        """
        Generate a problem prompt using PUCT archive for parent selection.

        Selects the next parent state from the PUCT archive, then builds
        the prompt using the standard prompt sampler.

        Returns:
            (prompt_dict, parent_program)
        """
        assert self.puct_archive is not None, "PUCT archive not initialized"

        # Select one state from PUCT archive
        selected_ids = self.puct_archive.select_states(batch_size=1)

        if not selected_ids:
            # Fallback to database sampling if archive is empty
            logger.warning("PUCT archive empty, falling back to database sampling")
            return self._problem_generator_database()

        selected_id = selected_ids[0]
        puct_state = self.puct_archive.get_state(selected_id)

        if puct_state is None or puct_state.code is None:
            logger.warning(f"PUCT state {selected_id} has no code, falling back to database sampling")
            return self._problem_generator_database()

        # Create a Program object from the PUCT state for prompt building
        parent = Program(
            id=puct_state.state_id,
            code=puct_state.code,
            language=puct_state.language,
            metrics=puct_state.metadata.get("metrics", {"combined_score": puct_state.reward}),
            parent_id=puct_state.parent_id,
            generation=puct_state.metadata.get("generation", 0),
            iteration_found=puct_state.metadata.get("iteration_found", 0),
        )

        # Build prompt using standard sampler (simplified for PUCT mode)
        prompt = self.prompt_sampler.build_prompt(
            ...
            inspirations=[],
            language=self.config.language,
            evolution_round=self.puct_archive.total_expansions,
    ...
    def _problem_generator_database(self) -> Tuple[Dict[str, str], Program]:
        """Original database-based problem generator (extracted for reuse)."""
    ...
    def problem_generator(self) -> Tuple[Dict[str, str], Program]:
        ...
        When PUCT reuse is enabled, delegates to problem_generator_puct().
        Otherwise, uses the standard database-based sampling.

        Returns:
            (prompt_dict, parent_program)
        """
        if self.use_puct_reuse and self.puct_archive is not None and self.puct_archive.size() > 0:
            return self.problem_generator_puct()
    ...
    # ---------------------------
    # PUCT Archive Management
    # ---------------------------

    def seed_puct_archive(self) -> None:
        """
        Seed the PUCT archive with the initial program.
        Should be called after initialize().
        """
        if self.puct_archive is None:
            return

        if not self._initialized:
            logger.warning("Cannot seed PUCT archive before initialization")
            return

        # Get the initial program from the database
        if self._initial_program_id in self.database.programs:
            initial_prog = self.database.programs[self._initial_program_id]
            reward = initial_prog.metrics.get("combined_score", 0.0)
            self.puct_archive.add_state(
                state_id=initial_prog.id,
                reward=reward,
                parent_id=None,
                is_seed=True,
                code=initial_prog.code,
                language=initial_prog.language,
                metadata={
                    "metrics": initial_prog.metrics,
                    "generation": initial_prog.generation,
                    "iteration_found": initial_prog.iteration_found,
                },
            )
            print(f"[PUCT] Seeded archive with initial program: reward={reward:.6f}")
        else:
            logger.warning(f"Initial program {self._initial_program_id} not found in database")

    def update_puct_archive_from_temp(self, verbose: bool = True) -> Dict[str, int]:
        """
        Update the PUCT archive from the temp cache of child programs.

        This should be called after each rollout to add newly evaluated
        programs to the PUCT archive and update expansion statistics.
        ...
        # Get temp programs (thread-safe copy)
        with self.database._temp_lock:
            temp_programs = self.database.temp_programs.copy()

        if not temp_programs:
            return stats

        # Group children by parent_id for batch update
        children_by_parent: Dict[str, List[Tuple[str, float]]] = {}

        for program in temp_programs:
            reward = program.metrics.get("combined_score", 0.0)
            ...
            # Track for parent expansion update
            if parent_id is not None:
                if parent_id not in children_by_parent:
                    children_by_parent[parent_id] = []
                children_by_parent[parent_id].append((program.id, reward))

        # Update expansion stats for each parent
        for parent_id, children in children_by_parent.items():
            self.puct_archive.update_after_expansion(parent_id, children)

        if verbose:
            best = self.puct_archive.get_best_state()
            best_reward = best.reward if best else 0.0
            print(f"[PUCT] Archive update: added={stats['added']}, skipped={stats['skipped']}, "
                  f"archive_size={self.puct_archive.size()}, "
                  f"best_reward={best_reward:.6f}, "
                  f"total_expansions={self.puct_archive.total_expansions}")
