...
class RolloutManager:
    ...
    def _post_process_rewards(self, samples: Union[list[Sample], list[list[Sample]]]):
        ...
        # Entropic and PKPO objectives handle their own advantage computation;
        # skip group normalization to pass raw rewards through.
        if self.args.advantage_estimator in ("entropic", "pkpo"):
            return raw_rewards, raw_rewards

        if (
            self.args.advantage_estimator in ["grpo", "dr_grpo", "gspo", "reinforce_plus_plus_baseline"]
            ...
            if self.args.advantage_estimator in ["grpo", "dr_grpo", "gspo"] and self.args.grpo_std_normalization:
