...
async def generate_rollout_async(
    ...
    if getattr(args, "evolving_gym", False):
        ...
        # Update PUCT archive from temp cache (before flushing to main database)
        if getattr(gym, "use_puct_reuse", False) and getattr(gym, "puct_archive", None) is not None:
            puct_stats = gym.update_puct_archive_from_temp(verbose=True)
            print(f"[SUCCESS] Rollout {rollout_id} PUCT archive update: {puct_stats}")
