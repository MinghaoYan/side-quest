    ...
    def add_slime_arguments(parser):
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=int,
                default=4,
                help="The k in pass@k for PKPO. Must satisfy 2 <= k <= n_samples_per_prompt.",
            )
            parser.add_argument(
                "--pkpo-estimator-type",
                type=str,
                choices=["s", "sloo", "sloo_minus_one"],
                default="sloo_minus_one",
                help="PKPO estimator variant. sloo_minus_one is recommended for lowest variance.",
            )
            parser.add_argument(
                "--pkpo-k-anneal-step",
                type=int,
                default=None,
                help="Step at which to anneal pkpo_k to pkpo_k_anneal_target. None disables annealing.",
            )
            parser.add_argument(
                "--pkpo-k-anneal-target",
                type=int,
                default=1,
            ...
            parser.add_argument(
                ...
                help="KL divergence constraint gamma for adaptive beta in the entropic objective (default: ln(2) ≈ 0.693).",
            )
            parser.add_argument(
        ...
        def add_custom_arguments(parser):
            ...
            # TTT-Discover PUCT reuse arguments
            parser.add_argument(
                "--use-puct-reuse",
                action="store_true",
                default=False,
                help="Use PUCT-based state reuse instead of MAP-Elites database sampling (TTT-Discover).",
            )
            parser.add_argument(
                "--puct-exploration-coef",
                type=float,
                default=1.0,
                help="PUCT exploration coefficient c (default: 1.0).",
            )
            parser.add_argument(
                "--puct-archive-size",
                type=int,
                default=1000,
                help="Maximum number of states in the PUCT archive (default: 1000).",
            )
            parser.add_argument(
                "--puct-top-children-per-parent",
                type=int,
                default=2,
...
def slime_validate_args(args):
    ...
    if args.advantage_estimator == "pkpo":
        ...
        if args.pkpo_k_anneal_step is not None:
            assert args.pkpo_k_anneal_target >= 1, (
                f"pkpo_k_anneal_target must be >= 1, got {args.pkpo_k_anneal_target}"
            )
