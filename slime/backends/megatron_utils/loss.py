...
def compute_advantages_and_returns(args, rollout_data):
    ...
    elif args.advantage_estimator == "entropic":
        rewards = torch.tensor(rewards, dtype=torch.float32, device=kl[0].device)
        print(f"[DEBUG] Entropic rewards tensor stats: min={rewards.min().item():.6f}, max={rewards.max().item():.6f}, mean={rewards.mean().item():.6f}")

        entropic_kl_constraint = getattr(args, "entropic_kl_constraint", 0.6931471805599453)  # ln(2)
        advantages, entropic_stats = get_entropic_returns(
            rewards=rewards,
            kl=kl,
            n_samples_per_prompt=args.n_samples_per_prompt,
            kl_coef=args.kl_coef,
            gamma_kl=entropic_kl_constraint,
        )
        returns = [a.clone() for a in advantages]

        # Log entropic-specific stats
        if entropic_stats.get("betas"):
            avg_beta = sum(entropic_stats["betas"]) / len(entropic_stats["betas"])
            print(f"[DEBUG] Entropic avg beta={avg_beta:.4f}, "
                  f"min beta={min(entropic_stats['betas']):.4f}, "
                  f"max beta={max(entropic_stats['betas']):.4f}")

    elif args.advantage_estimator == "pkpo":
        rewards = torch.tensor(rewards, dtype=torch.float32, device=kl[0].device)
        print(f"[DEBUG] PKPO rewards tensor stats: min={rewards.min().item():.6f}, max={rewards.max().item():.6f}, mean={rewards.mean().item():.6f}")

        pkpo_k = getattr(args, "pkpo_k", 4)
        pkpo_estimator_type = getattr(args, "pkpo_estimator_type", "sloo_minus_one")

        # Handle k annealing: reduce k after a specified step
        pkpo_k_anneal_step = getattr(args, "pkpo_k_anneal_step", None)
        pkpo_k_anneal_target = getattr(args, "pkpo_k_anneal_target", 1)
        current_step = rollout_data.get("rollout_id", 0)
        if pkpo_k_anneal_step is not None and current_step >= pkpo_k_anneal_step:
            effective_k = pkpo_k_anneal_target
        else:
            effective_k = pkpo_k

        # k=1 means standard pass@1, no PKPO transform needed
        if effective_k <= 1:
            returns = get_grpo_returns(rewards, kl)
            advantages = [r for r in returns]
            print(f"[DEBUG] PKPO annealed to k=1, using standard GRPO returns")
        else:
            advantages, pkpo_stats = get_pkpo_advantages(
                rewards=rewards,
                kl=kl,
                n_samples_per_prompt=args.n_samples_per_prompt,
                k=effective_k,
