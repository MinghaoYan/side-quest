#!/bin/bash
# Top-level script for running LLMSR with SLIME + PACEvolve.
#
# Usage: bash run_llmsr.sh
...
#### LLMSR task config selection ####
# Maps to pacevolve/tasks/llmsr/config/config_{LLMSR_CONFIG_ID}.yaml
LLMSR_CONFIG_ID=1
...
IS_TRAINING=False
LLMSR_MAX_ITERS=80
LLMSR_EVAL_TIMEOUT=600
LLMSR_RECOMPILE_TIMEOUT=120
...
export PACEVOLVE_NUM_ROLLOUT=256
export PACEVOLVE_N_SAMPLES_PER_PROMPT=2
export PACEVOLVE_ROLLOUT_MAX_RESPONSE_LEN=8192
export PACEVOLVE_MAX_CONCURRENT_EVALS=1
...
#### Dataset location ####
# Must contain:
#   - llm-srbench-lsr_synth_phys_osc.arrow
#   - lsr_bench_data.hdf5
LLMSR_DATA_PATH="${LLMSR_DATA_PATH:-/workspace/pacevolve/tasks/llmsr/data}"
...
TASK_ROOT="${ROOT_DIR}/pacevolve/tasks/llmsr"
BASE_CONFIG_YAML="${TASK_ROOT}/config/config_${LLMSR_CONFIG_ID}.yaml"

if [ ! -f "${BASE_CONFIG_YAML}" ]; then
    echo "Missing LLMSR base config: ${BASE_CONFIG_YAML}"
    exit 1
fi

if [ ! -f "${LLMSR_DATA_PATH}/llm-srbench-lsr_synth_phys_osc.arrow" ] || [ ! -f "${LLMSR_DATA_PATH}/lsr_bench_data.hdf5" ]; then
    echo "LLMSR dataset files not found under: ${LLMSR_DATA_PATH}"
    echo "Expected files:"
    echo "  - llm-srbench-lsr_synth_phys_osc.arrow"
    echo "  - lsr_bench_data.hdf5"
    echo "Set LLMSR_DATA_PATH=/path/to/llmsr/data and rerun."
    exit 1
fi

RUN_NAME="${SMALL_MODEL_NAME}_pacevolve_llmsr_cfg${LLMSR_CONFIG_ID}${POSTFIX_STR}"
...
RUNTIME_CONFIG_YAML="${SAVE_PATH}/${RUN_NAME}/config_llmsr_runtime.yaml"
python3 - "${BASE_CONFIG_YAML}" "${RUNTIME_CONFIG_YAML}" "${TASK_ROOT}" "${LLMSR_DATA_PATH}" "${LLMSR_MAX_ITERS}" "${LLMSR_EVAL_TIMEOUT}" "${LLMSR_RECOMPILE_TIMEOUT}" <<'PY'
import os
import sys
import yaml
...
echo "=== LLMSR + PACEvolve Configuration ==="
echo "RUN_NAME:     ${RUN_NAME}"
echo "CONFIG_YAML:  ${RUNTIME_CONFIG_YAML}"
echo "DATA_PATH:    ${LLMSR_DATA_PATH}"
echo "SAVE_PATH:    ${SAVE_PATH}"
echo "SEED:         ${SEED}"
echo "IS_TRAINING:  ${IS_TRAINING}"
echo "MAX_ITERS:    ${LLMSR_MAX_ITERS}"
